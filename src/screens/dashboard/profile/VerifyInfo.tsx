...
type route = RouteProp<HomeScreenParam, 'VerifyInfo'>;

const CELL_COUNT = 6;

const VerifyInfo: React.FC = () => {
  const appColor = useAppColor();
  const [otp, setOtp] = useState('');
  const [seconds, setSeconds] = useState<number>(30);
  const {navigate, goBack} = useNavigation<nav<HomeScreenParam>>();
  ...
  return (
    <ViewContainer style={{flex: 1, backgroundColor: appColor.white}}>
      ...
      <ScrollView style={{flex: 1}}>
        <View>
          <Paragraph fontWeight="bold" fontSize={18} color={appColor.black}>
            {`Verify ${
              params.type === 'email' ? 'Email Address' : 'Phone Number'
            }`}
          </Paragraph>
          <Paragraph mt={5}>
            {`Enter the 6 digit pin sent to ${params?.data ?? ''} `}
          </Paragraph>
          <Spacer height={40} />
          <CodeField
            ref={ref}
          ...
          <Spacer height={50} />
          <AppButton text="Proceed" onPress={() => {}} variant="primary" />
        </View>
      </ScrollView>
    </ViewContainer>
  );
